...
2026-09-23 编写，2026-09-24 更新。适用于候选 web commit `8f47a1289bf501616ba234638192a5303d3155d7`。**不要**拿现有 `/tmp` 假身份、空 API fixture、原型 63 条静态对照、headless 截图或已构建的 dist 冒充真实服务验收。测试账号口令仅在授权登录页输入，不写入脚本、日志、URL、截图或本库。
...
### 2026-09-24 本任务执行口径补充
- 以上为原开发候选的历史检查单；本次依用户当前指令不创建独立Reviewer，采用已授权本地构建部署并真实服务验证。18份相关测试265 PASS不是全仓测试，也不冒充固定Chrome133/实体手机验证。
- 实际发布为合并候选cb73a25（包含线上正式流程修复），Web1.13.24已健康发布。用户继续授权同#397、吴用、真实Provider费用；仍禁止伪造状态和越权操作。
- 11:28确证API进程被内核OOM终止，首轮新页面真实显示网络失败；前端静态资源健康不可代替后端业务健康。构建已结束、关闭本任务Chromium，canonical launcher恢复同exact API22+BF14配置后再继续，不回滚改版/不操作其他任务进程。
