...
## 逐条复核与判定口径（2026-09-23 初核，2026-09-24 补核）
...
补充本地 **200% 实际浏览器缩放**预检：4 个缩放后视口均通过七入口切换（六个类名与源码对应的业务面板容器加回概览）、典籍检索页签选取、面板底界/页面横溢和标题语义检查，见 [`actual-zoom-review.md`](evidence/actual-zoom-review.md)；该取证仍是隔离空 API，**不提高真实写流程的证据等级**。
