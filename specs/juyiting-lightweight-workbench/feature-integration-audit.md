...
**对“议事内容太少”的内容态再核验**：在不接入生产服务的隔离浏览器中用真正的 Vue 会话 API 路径渲染 28 条合成 Markdown，100% 手机两视口和 200% 四视口均确认消息区独立纵向滚动、气泡不越界、代码块内部横滚、底栏不遮输入（极短屏可滚到发送）；逐项字体、尺寸、颜色及示例链接见 `evidence/rendered-chat-review.md`。这补齐了之前仅空会话和 DOM 插入段落的布局证据，**不补齐真实会话读取、SSE 及发送**。
