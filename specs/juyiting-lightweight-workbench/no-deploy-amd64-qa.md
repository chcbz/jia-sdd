...
2026-09-23。本文件是**操作前检查单**，不是运行报告或通过证明。目标提交仅限 `cyf-web-kit` 的 `fe43ebc5bece2fd2e6e78e765cd523d9fb6001a5`（远端 `codex/juyiting-lightweight-workbench` 已核对）；不得把当前 `develop`、旧 Flow Run 147 或本机 Chromium 142 的结果归给该提交。API 源码未改，根仓库 `web` gitlink 尚未 pin；详见 `acceptance.md`。
...
test "$(git rev-parse HEAD)" = fe43ebc5bece2fd2e6e78e765cd523d9fb6001a5
npm ci
