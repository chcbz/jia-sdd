...
历史一次以默认 2 秒超时执行时出现 before-all 初始化超时，未计为业务断言失败。缺失的指定 Chromium/地图高耗时测试仍列为待复核；早前地图 pageerror 已按 A1 定位并在新实例复验消失。
