...
- 历史原型 63 条覆盖结论属于 Demo；新正式组件接入和仍需实际验证的差异分别见 `feature-integration-audit.md`、`ui-detail.md`。`docs/ui-workbench/ui-detail.md` 是原型属性详设，**实现态以本目录及 live fixture 样本为准**。验收完成前不得将 `integration.yaml` 标为 accepted；`./sddw pin` 会提前改成 integration-ready，真实服务/受控浏览器门禁通过前不要执行。Web 提交已推至非发布特性分支，但 root 的 `web` gitlink 仍保持原值。前序独立只读复核对 `a935440` 的菜单和详设抽查无 P0/P1；菜单证据 P2 已补候选 PNG/定位祖先约束，原始工作区的另外两项文档 P2 属于其他未提交用户改动，不跨工作树修改。现候选 `8f47a12` 的极窄/极短复排已独立只读复核，无 P0/P1：七份测试 146 passing、四视口数值和截图重产一致；发现脚本原先未校验可见文字/本地 SHA 的两项 P2 后已修并重新跑通。独立审查未重跑 `npm run build`，先前构建成功仍仅是本地验证记录；受控 amd64 门禁、候选部署/真实写入仍缺。
