...
范围限制：后续BF15–18、BF24–25及实体设备/全产品验收未冒称完成，见`../juyiting-formal-flow-bugfix-20260923/remaining-bugfixes.md`；下面保留旧版本失败/待验的历史，不代表当前#397状态。
...
## 2026-09-24 当前版本合入（本地例外授权）
- 按用户本次明确指令，将Web改版8f47a12真实合入当前线上244f944，保留BF07–BF13状态/身份/正式执行修复；不把交接文档中的旧API SHA覆盖当前API22+BF14配置。
- 当前执行允许本地测试、构建和例外发布；不创建Reviewer。历史设备/Flow结果保留为历史，不替代本次结果。
- 两个测试冲突保留两侧目的；挂载测试补齐最新TaskMaterialLinks别名和正式scope依赖后复测。
- 唯一执行台账仍为TASKS.yaml；精确合并/测试/部署证据在deliverables/verification/juyiting-workbench-integration-20260924。尚未发布改版，尚未通过正式闭环。

### 2026-09-24 后续联调
改版保持在已上线Web1.13.26（1bff9ca，为改版双父merge cb73a25后代）。移动430×932触控浏览器已验证概览→原榜文→原议事→选定吴用→正式验收→精确PDF返工表单。#397 r1 PDF五页预览通过、要求修改200；返工POST新发现404，API1.13.27已修正源成果INPUT事务关联并通过48项测试，待部署。业务全流程仍未通过，不能把构建/页面改版上线当作可验收。
