...
状态：OD00～OD05 开发门槛与独立评审已通过；OD06 客户端保留清理已通过独立评审，API 合并及定向回归进行中，真实联调与正式打包尚未验收，尚未部署。实际状态与证据见 [task-ledger.yaml](task-ledger.yaml)；依赖 [detailed-design.md](detailed-design.md) 与 [tasks.md](tasks.md) 的逐项任务。
