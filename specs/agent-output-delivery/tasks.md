...
实施状态：全部未启动。每项只有一个写入者，验证后独立只读评审并依项目规则提交用户确认。流程与命令见 [execution-plan.md](execution-plan.md)，技术契约见 [detailed-design.md](detailed-design.md)。
...
- [ ] 提交具体commit、测试结果、风险和演示供确认；然后更新任务状态。
