...
- `smoke-output-browser.mjs` retains exact resource-route checks and adds optional `entryMode: "hall-bounty"` with the fixture's `bountyStatus`. It opens the actual desktop bounty-map hotspot or mobile portrait button, filters/searches by exact task ID, opens details, closes/reopens, previews and verifies downloaded bytes. It reads canonical map geometry but never calls internal panel/dispatch methods. Navigation failures capture bounded DOM diagnostics and a screenshot; execution/review attempts are blocked. The new mode has only syntax validation and failed live diagnostics so far: desktop navigation reached real artifact data, but nested dialog clipping prevented closing/details access (OD11/hall-bounty-navigation-r3). No successful download or passed viewport is claimed for this new mode; product layout repair and rerun are pending. Old resource-route evidence is unchanged.

- `smoke-output-lease.py` is the OD07 lease probe, independently accepted on8a908ae after bounded repairs, but not yet live-validated. It requires an already trusted READY policy1 work item and `OUTPUT_SMOKE_RUN_TICKET`. Only numeric loopback HTTP10018 is accepted. GET → claim/start/heartbeat/release uses returned string versions and exact receipt replays, with bounded/schema-checked responses and hash-only sensitive response evidence. This bounded probe requires every action to advance exactly one version, including an actually renewing heartbeat; a valid no-op heartbeat is not renewal evidence for this probe. Success releases the lease; a failure stops immediately and may leave an active lease for the operator to inspect/expire, without guessing new versions or automatically retrying another operation. It never creates tasks/runs, executes a model, submits a delivery or provides concurrency proof. `PYTHONDONTWRITEBYTECODE=1 python3 specs/agent-output-delivery/tools/test-lease-probe-controls.py` passed seven synthetic helper-control tests after repair; these do not contact CYF and are not OD07 product test evidence.
