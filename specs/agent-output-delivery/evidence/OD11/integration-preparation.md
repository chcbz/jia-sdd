...
The task-only0771f107 starter was deliberately stopped after paging verification to release roughly850MiBRSS while OD08 Gradle/MySQL tests run; writer confirmed it has no10018 dependency. pidfd, startTicks and exact immutable jar were checked; record isolated-starter-stopped-after-paging.json. Current10018 is no longer an active runtime. Reuse preserved config/jar only through a new launch observation namespace, or start the next frozen candidate. MySQL/MinIO/Rabbit/ClamAV and user Agent remain unchanged.
