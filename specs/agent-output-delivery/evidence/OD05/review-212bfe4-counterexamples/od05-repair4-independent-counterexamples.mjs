...
const List=compile('OutputList',[[/^import\s+OutputCard.*$/gm,'var OutputCard = dep0',Card],[/^import\s+OutputPreview.*$/gm,'var OutputPreview = dep1',Preview],[/^import\s+\{\s*parseOutputResourceQuery\s*\}\s+from.*$/gm,'var parseOutputResourceQuery = dep2',parseOutputResourceQuery]])
{
 ...
 const {wrapper}=harness(source,{requestedResource:request,http:{get:async(url)=>{calls.push(url);return url.endsWith('/outputs')?page([]):{data:{data:{item:item('report',url.split('/').at(-1))}}}}}},List)
...
{
 const source=ref(sourceData),request=ref({source:sourceData,outputId:'revoked-secret',version:'1'});let revoked=false
 const {outputs,wrapper}=harness(source,{requestedResource:request,http:{get:async(url)=>{
  if(url.endsWith('/outputs'))return page(revoked?[]:[item('revoked-secret')])
  if(revoked)throw Object.assign(new Error('No longer accessible'),{status:404,retryable:false})
  return {data:{data:{item:item('revoked-secret')}}}
 }}},List)
 await flush();revoked=true;await outputs.refresh();await flush()
 console.log('FOCUSED_RESOURCE_AFTER_AUTHORITATIVE_EMPTY_REFRESH',JSON.stringify({items:outputs.items.value,rendered:wrapper.text(),cards:wrapper.findAll('.output-card').length}));wrapper.unmount()
}
{
 const source=ref(sourceData),request=ref({source:sourceData,outputId:'source-revoked-secret',version:'1'});let revoked=false
 const {outputs,wrapper}=harness(source,{requestedResource:request,http:{get:async(url)=>{
  if(revoked)throw Object.assign(new Error('Source forbidden'),{status:403,retryable:false})
  if(url.endsWith('/outputs'))return page([item('source-revoked-secret')])
  return {data:{data:{item:item('source-revoked-secret')}}}
 }}},List)
 await flush();revoked=true;await outputs.refresh();await flush()
 console.log('FOCUSED_RESOURCE_AFTER_SOURCE_403',JSON.stringify({items:outputs.items.value.map(x=>x.title),rendered:wrapper.text(),cards:wrapper.findAll('.output-card').length}));wrapper.unmount()
}
{
 const source=ref(sourceData),request=ref({source:sourceData,outputId:'valid-history',version:'1'});let details=0
 const {outputs,wrapper}=harness(source,{requestedResource:request,http:{get:async(url)=>{
  if(url.endsWith('/outputs'))return page([])
  details++;return {data:{data:{item:item('valid-history')}}}
 }}},List)
 await flush();const before=details;await outputs.refresh();await flush()
 console.log('VALID_HISTORICAL_RESOURCE_REVALIDATED',JSON.stringify({before,after:details,rendered:wrapper.text(),cards:wrapper.findAll('.output-card').length}));wrapper.unmount()
}
{
 const source=ref(sourceData),request=ref({source:sourceData,outputId:'history-after-recovery',version:'1'});let revoked=false,hold=false,resolveHeld,details=0
 const {outputs,wrapper}=harness(source,{requestedResource:request,http:{get:async(url)=>{
  if(url.endsWith('/outputs')){
   if(revoked)throw Object.assign(new Error('Source forbidden'),{status:403,retryable:false,requestId:'denial-evidence'})
   return page([])
  }
  details++
  if(hold)return new Promise(resolve=>{resolveHeld=resolve})
  return {data:{data:{item:item('history-after-recovery',url.split('/').at(-1))}}}
 }}},List)
 await flush();hold=true;const pending=outputs.detail(item('history-after-recovery')).catch(e=>e);await flush()
 revoked=true;await outputs.refresh();await flush()
 resolveHeld({data:{data:{item:item('history-after-recovery')}}});const late=await pending
 const callsAtDenial=details;request.value={...request.value,version:'2'};await flush()
 const denied={lateResult:late?.name,detailCallsBefore: callsAtDenial,detailCallsAfter:details,cards:wrapper.findAll('.output-card').length,error:outputs.error.value}
 revoked=false;hold=false;await outputs.refresh();await flush()
 console.log('SOURCE_DENIAL_LATE_RESPONSE_FOCUS_CHANGE_AND_RECOVERY',JSON.stringify({denied,recovered:{error:outputs.error.value,cards:wrapper.findAll('.output-card').length,text:wrapper.text(),details}}));wrapper.unmount()
}
