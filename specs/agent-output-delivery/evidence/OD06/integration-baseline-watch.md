...
## Broader regression boundary — 2026-09-12

The last broader four-module command stopped at agent-service:test:1539 tests,6 failures,121 skips, exit1 (10m44s). It did not establish a four-module PASS. Five failures concern merged constructor annotations and test-stage/event-writer contracts; the sole writer is repairing and targeting them. AgentCommandDlqRedriverRabbitIntegrationTest instead requires exact /home/isp/apps/rabbitmq RabbitMQ3.6.11 and snapshots a historical production PID8150/state. That server is absent on this host; current isp-install references3.12.12/Erlang26.2.1. Do not fake the historical path/version or convert the test to skipped. Preserve this unavailable D09 host gate separately from actual R1 output checks, and have the independent reviewer assess the resulting development/release boundary.
