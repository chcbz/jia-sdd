...
| Web | `84810522d33adb377dec77162fc15051e19526fe` | `833da6a7473fd4aebd1e137a92e50fdc6fc0b789` | MATCH |
...
- `displayName` 原生断言 PASS：正常“陈惠超”、完整 raw mojibake、percent UTF-8、合法西文名、emoji、不可逆 fallback、稳定 jiacn owner 判定。
- `node --check src/utils/displayName.js` 与相关 Hall JavaScript：PASS。
- `git diff --check`：PASS。
- 隔离工作树无 `node_modules`，因此尚未把 Mocha/Vite 结果冒充已执行。

## 5. Flow 状态

北京时间 2026-09-26 12:22 查询：

- API `5260799` 最新仍为历史 Run 96 FAIL，source commit 未返回；未出现绑定 API `188bf0c0...` 的新 Run。
- Web `4403172` 最新仍为历史 Run 147 FAIL；未出现绑定 Web `84810522...` 的新 Run。
