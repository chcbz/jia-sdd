...
- [ ] API：用户资料、典籍/书签、聊天、任务的 owner 谓词；严格版本不读取旧 tenant 格式，并补充交叉 owner 拒绝测试。
...
- [ ] DB：生产只读盘点生成完整 tenant 表清单和不可迁移记录的精确主键删除集；加法 DDL、私有快照、临时 DML/删除、读回和回滚脚本分离。受控 owner 参数不落库日志或 Git。
...
- [ ] Release：维护窗口、生产迁移/删除快照与读回、严格版本 Flow、线上 smoke。
