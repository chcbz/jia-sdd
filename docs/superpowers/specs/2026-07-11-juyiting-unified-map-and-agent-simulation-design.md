...
日期：2026-07-11
状态：已确认，作为后续实施的唯一主规格；2026-07-19 已恢复至根 `docs/`，当前执行进度见本文“第 18 章 当前实施进度与后续任务”。
...
> 更新时间：2026-07-19
>
> 本节以当前前端 `web`、后端 `api` 的已提交代码和现网开关状态为准。它不替代本文第 1 至 15 节的架构约束；发生冲突时，以本文的核心原则、状态优先级和 TMX 唯一事实源约束为准。

### 18.1 当前结论

聚义厅的 TMX movement、前端模拟引擎、后端语义场景状态、快照/SSE/phase 回报的基础设施均已完成第一阶段实现；但“TMX 路网本地巡逻 + 后端业务状态抢占 + 生产灰度”的端到端闭环尚未启用。

当前线上宋江仍使用普通前端巡逻：`useHallScene` 根据区域和 seed 临时生成小范围 `patrolRoute`，`HallAgent` 用显式 `dt` 积分推进。该临时路线不是本规格定义的 TMX 路网巡逻，属于待替换的兼容实现。

前端 `VITE_JUYITING_SIMULATION_ENABLED` 当前默认关闭。宋江一旦启用 simulation 会停止普通巡逻并等待 scene snapshot；在后端未提供有效宋江状态时，人物会静止。因此不得仅修改前端环境变量后直接在生产开启。

### 18.2 已完成

| 项目 | 状态 | 证据与说明 |
| --- | --- | --- |
| TMX movement schema、可走区域、路网、region、home/parking/queue slot | 已完成 | `public/juyiting/hall.tmx` 与 movement parser/validator 已进入前端代码和测试。 |
| TMX 路网 A*、slot reservation、movement runtime | 已完成 | 前端已有 movement graph、pathfinder、slot allocator 与 movement engine。 |
| 单人物 simulation command、路径推进、到达/阻塞 phase | 已完成 | engine 支持 `enqueue`、`cancel`、snapshot、phase 回报和版本水位线。 |
| 后端 scene snapshot、SSE、幂等 phase report | 已完成 | `/agent/scenes/juyiting-main/{snapshot,events,phases}` 已有 DTO、持久化、版本与测试。 |
| 后端业务语义状态发布 | 已完成基础能力 | 后端可从现有业务操作写入 `originRegionId`、`targetRegionId`、`behavior`、`stateVersion` 等语义状态。 |
| 前端 scene state 到 simulation bridge | 已完成 | 后端 snapshot/SSE 可转换为 `source: 'backend'` 的移动命令；同一 `agentId` 会复用现有实体。 |
| 普通巡逻跨点反复横跳修复 | 已完成 | 非 simulation 巡逻采用显式位置积分和到点钳制，避免 melonJS Body 速度重复位移。 |

### 18.3 未完成或不符合主规格的部分

| 项目 | 当前状态 | 与主规格的差异 |
| --- | --- | --- |
| 本地巡逻沿 TMX 路网运行 | 未完成 | 当前普通巡逻由前端生成区域内小圈路线，未发出 `local` command 并沿 `nav_nodes` / `nav_edges` A* 行走。 |
| 自动巡逻生命周期 | 未完成 | simulation engine 为命令驱动；没有“候命后开始巡逻、抵达后继续下一段、业务结束后恢复巡逻”的状态机。 |
| 后端宋江初始 scene state | 不适用 | 本地 TMX 巡逻是候命状态的权威，不应伪造后端坐标/路径状态；后端仅在真实业务移动时发布语义状态。 |
| 业务抢占巡逻的真实链路 | 未完成 | 版本化命令和优先级基础存在，但没有在“TMX 本地巡逻正在运行”时以真实业务状态完成验收。 |
| 生产灰度 | 未启用 | 前后端 scene/simulation 开关仍保持关闭，避免空状态导致宋江静止。 |
| 多人物避让与重规划 | 未完成 | 属于本文第二阶段范围，不应与宋江单人物闭环混合上线。 |

### 18.4 后续实施顺序

#### Task 1：固化 TMX 本地巡逻定义

- 在 `hall.tmx` 的 movement 数据中补充宋江可用的巡逻路线/节点分组和停留配置；路线只能经过现有可走路网。
- 解析层输出稳定的 `patrolRouteId`、起始节点、候选节点、循环规则和停留时间；不把巡逻坐标复制到后端。
- 增加 parser、validator 和 graph reachability 测试：路线节点存在、相邻节点可达、目标 slot 不被误用为巡逻点。

**完成标准：** 从 TMX 读取同一条宋江巡逻定义，在不同刷新、不同浏览器中得到相同的 local command 路径输入。

#### Task 2：将普通巡逻迁移为 local simulation command

- 宋江空闲时不再生成 `buildPatrolRoute()` 小圈路线。
- 前端根据 TMX `patrolRouteId` 创建 `source: 'local'`、低优先级的 movement command；movement engine 统一推进位置、动画、到达与阻塞。
- 抵达一个巡逻节点后按 TMX 定义停留，再创建下一段 local command；不得直接操作实体坐标或 melonJS Body 速度。
- 保留其他未接入 simulation 的人物当前普通巡逻，避免扩大本次改动范围。

**完成标准：** 宋江连续多帧只有一个实体，沿 TMX 路网巡逻；刷新后可由当前 local state 恢复到合理阶段；无横跳、镜像、重影或越界。

#### Task 3：接入后端业务状态抢占与恢复

- 后端业务操作继续只写语义状态，不写路径、坐标或动画帧。
- scene state 到达前端时，以更高 `stateVersion` 的 `source: 'backend'` command 抢占 local patrol；遵守本文 5.9 与 8.3 的 committed 动作规则。
- phase `arrived` / `blocked` 回报保持幂等；过期回报必须被拒绝，不能恢复旧巡逻。
- 业务状态结束、超时或人物回到 home 后，前端恢复指定 TMX local patrol；恢复点以当时实际位置为起点重新规划。

**完成标准：** 巡逻中派发任务后，宋江仅转为一次业务移动；完成或取消后只恢复一条 TMX 巡逻，不生成第二实体或执行旧命令。

#### Task 4：补齐初始状态、灰度与可观测性

- 后端为 `juyiting-main` 提供可验证的宋江初始候命/业务 state；没有状态时前端必须保留明确降级，不可静默停止人物。
- 增加前后端独立开关和诊断字段：simulation ready、snapshot state count、active command、stateVersion、最近 phase 结果、降级原因。
- 灰度顺序固定为：本地注入 → 测试环境真实 snapshot/SSE → 单租户/单宋江生产灰度 → 扩展更多人物。
- 任一阶段出现空快照、版本断档、路网不可达或重复实体时，关闭 simulation 开关并回退到当前已验证的普通巡逻。

**完成标准：** 生产灰度的宋江有有效 scene state、单实体、可连续移动；关闭开关后页面不红屏、不黑边、不重影，且可恢复普通巡逻。

### 18.5 验收矩阵

| 场景 | 必须验证 |
| --- | --- |
| TMX 巡逻 | parser/validator、路网可达性、路线不穿障碍、节点停留、连续多帧单实体。 |
| 本地巡逻 | 低帧率、刷新、横竖屏切换、拖动地图、最小/最大缩放时位置连续。 |
| 业务抢占 | 巡逻中下达业务、committed 临界点、重复/旧 stateVersion、取消与业务结束恢复。 |
| 状态同步 | snapshot 初始加载、SSE 重连、版本断档 resync、重复 phase、blocked 重规划。 |
| 视觉与交互 | canvas 无红屏/黑边/重影；热点可点击；人物点击不影响移动；同帧仅一个宋江。 |
| 回滚 | 分别关闭前端 simulation、后端 scene state/SSE；确认降级路径可用且业务接口不受影响。 |

### 18.6 变更守则

1. `hall.tmx` 继续是地图和 movement 数据唯一事实源；不得在后端复制巡逻坐标或路线。
2. 后端只表达业务语义、优先级和版本；不得要求前端按后端帧级坐标移动。
3. 同一实体只能有一个 active movement command；普通巡逻、local simulation 与 backend simulation 不得并行推进坐标。
4. 所有 simulation 接管必须复用真实 `agentId`（宋江为 `builtin-songjiang`），禁止以“模拟实体”方式复制人物。
5. 每次改动先跑对应单元测试，再做真实浏览器连续帧验证；视觉问题不得只凭单帧截图判断。
6. 生产开关只能在完整 snapshot/SSE/phase 闭环通过后开启；任何空状态均视为阻断上线问题。
