...
**用户已明确授权的12表修复完成，本轮认证API40/40、真实Chromium15/15通过；可进行经济只读预览的用户验收。用户最终验收仍待回执，不等于全站所有场景已验证。**

- foundation5+marketplace7表仅执行固定新增DDL。列、索引、外键、CHECK、引擎、排序规则验证符合canonical；未修改已有业务数据、未创建另外2张托管表、未开启收费/交易/语音。
- 18:40创建完成，18:41只读结构校验完成；MySQL将`OCTET_LENGTH`规范化为同义`LENGTH`造成比较器差异，归因后只修正任务私有比较器，**没有重复DDL**，原失败证据保留。
- 钱包/流水/技能目录/4位本人Agent权益均由503恢复200/E0；预算试算、托管配置参考、本地Agent租约不适用、匿名401/未知资源404/非法Origin403、聚义厅与典籍相关只读流程通过。
- 页面五区、余额、空目录、本人Agent选择、试算、横竖屏可滚动、聚义厅4位好汉/招贤令可见均实际通过。新表没有业务数据，所以余额0、无流水/商品/权益是正常结果，不是接口失败；未伪造商品或安装证据。
- 12表回归前后exact COUNT(*)均0，结构未变。该证据只证明这些表的前后行数不变，不扩大为全库零写入或无瞬时写入的证明。
- 运行API PID/start_ticks/JAR均未变化，健康UP。本轮没有重建、重新部署或重启；前后端仍是本页列示的1.7.0 exact提交/制品。
- 最新授权/安装交付：`/home/isp/wsps/cyf/deliverables/releases/v1.7.0-schema-repair-authorized-20260917/`；原未授权候选与首次失败发布包不回写。
- 回归原始证据：`/var/tmp/cyf-v1-7-post-schema-regression-20260917/api-report.json`、`browser-r2/browser-report.json`；计数证据：`/var/tmp/cyf-v1-7-schema-install-20260917/counts-after-regression.json`，均已按原字节封存进上述交付目录。

- 18:50最终封存完成：`/home/isp/wsps/cyf/deliverables/releases/v1.7.0-schema-repair-authorized-20260917/schema-repair-final-result.json`，SHA-256 `04fbb2759b26a4bfe23d2ab336dba4ae256e2fd69da035c81bc7205460448cee`；`FINAL_EVIDENCE_INDEX.json`及52文件摘要复核通过。唯一台账中缺表修复为`done`、集成为`waiting_user_acceptance`，两项Owner均已释放；历史失败及归因保留。

### 用户验收入口与边界

登录站点的“经济预览”（`/economy-preview`）：查看钱包→填写预算并试算→检查技能目录空态→选择自己的Agent核对权益/证据→查看托管说明。试算不扣款、不下单，真实交易、安装、托管开通和语音仍未启用。可切换自己的另一账号复验隔离。

本轮线上使用用户已授权的一个测试账号；生产第二账号/另一client、非空商品详情及非空成果数据、真实手机和server托管租约正例没有新增成功证据。两张托管读表仍是单列未授权缺口，本次只证明配置参考/local分支，不隐瞒或扩大为server全分支已交付。历史任务378依旧没有当前可见夹具，404不计为成果正例通过。

### 浏览器回归工具归因

本轮首轮12PASS/2FAIL中，两项聚义厅可见性检查没有开启触控模拟；只设置mobile viewport不代表`pointer: coarse`，产品会按桌面全景模式运行。保留首轮报告，仅修正私有runner：R2明确设置并断言coarse pointer/maxTouchPoints，等待真实可见列表，最终15/15通过。未改产品、未mock响应、未把首轮失败直接改成PASS。与上一轮10秒过早采样分别归档。

## 18:15 首次发布结论（历史快照，已由下述授权修复解除503）
