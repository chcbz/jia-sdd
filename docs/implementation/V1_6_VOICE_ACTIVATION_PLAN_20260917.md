...
## 2026-09-17 最新决定：用户延期验收

用户已要求语音验收先延后，开始1.7.0经济预览。两个语音任务均转`deferred`，不是验收通过/归档，不再要求用户立即处理网关，也不继续探针、重建或启用入口。保留单次TTS404、STT未调用证据；后续明确重新排期且音频能力变化后再恢复。**不阻塞1.7开发。** 以下执行安排为延期前记录，当前不自动执行。

原用户要求：继续既定OpenAI语音实现，达到可交付验证时通知。此任务独立于已经完成的1.6.0代码发布，不把源码/普通health成功当成语音可用。
