...
activation_failure() {
  ...
  if [[ "$PHASE" == STOPPED || "$PHASE" == CANDIDATE_INSTALLED ]]; then
    ...
    if [[ "$BACKUP_CONFIG" == ABSENT ]]; then
      host_remove_durable "$VOICE_LIVE"
    else
      host_copy_exclusive "$BACKUP_CONFIG" "$RESTORE" 0600 "$PREVIOUS_SHA" "$CONFIG_UID" "$CONFIG_GID" && host_replace_durable "$RESTORE" "$VOICE_LIVE"
    fi
    if (( $? == 0 )) && host_call_lifecycle start >/dev/null 2>&1; then
      host_record_state "$RECORD" ROLLED_BACK_HEALTHY 2>/dev/null; host_finalize_record "$RECORD" 2>/dev/null
    else
      host_record_state "$RECORD" FAILED_MANUAL_RECOVERY_REQUIRED 2>/dev/null || true; host_finalize_record "$RECORD" 2>/dev/null || true
    fi
  else
    host_record_state "$RECORD" ROLLED_BACK_HEALTHY 2>/dev/null || true; host_finalize_record "$RECORD" 2>/dev/null || true
  fi
  exit "$rc"
}
...
PY
TTS_BODY="$SMOKE_DIR/tts.bin"; TTS_HEADERS="$SMOKE_DIR/tts.headers"
TTS_CODE="$(printf 'Authorization: Bearer %s\n' "$TOKEN" | "$CURL" --header @- --silent --show-error \
  --dump-header "$TTS_HEADERS" --output "$TTS_BODY" --write-out '%{http_code}' --request POST \
  --header 'Content-Type: application/json' --data '{"text":"聚义厅语音验收","voice":"juyiting-default","format":"mp3"}' \
  http://127.0.0.1:10018/chat/speech/synthesis)"
[[ "$TTS_CODE" == 200 && -s "$TTS_BODY" ]] || die "authenticated TTS smoke failed"
grep -Eiq '^Content-Type:[[:space:]]*audio/mpeg([[:space:]]*;.*)?[[:space:]]*$' "$TTS_HEADERS" || die "TTS smoke content type is invalid"
rm -rf -- "$SMOKE_DIR"; SMOKE_DIR=''
host_record_state "$RECORD" STARTED_HEALTHY; host_record_state "$RECORD" COMMITTED; host_finalize_record "$RECORD"
